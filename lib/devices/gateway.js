...
class Gateway extends Device {
	...
	set_color(brightness, rgb){
		if(!this.devApi || !this.devApi.token || !this._developerKey){
			return
		}
		const b = Math.max(0, Math.min(255,parseInt(brightness)))
		const color = rgb & 0xffffff
		const combined = (b<<24) | color
		...
		const data = {
			...
			short_id:"0",
			data: { rgb: combined,
				key: this.devApi.getSendKey(
						this.devApi.token,
						this._developerKey) }
		}
		this.devApi.send(data);
	}
...
class DeveloperApi extends EventEmitter {
	...
	getSendKey(token, key){
		const iv = new Buffer('17996D093D28DDB3BA695A2E6F58562E', 'hex');

		var cipher = crypto.createCipheriv('aes-128-cbc', key, iv);
		cipher.setAutoPadding(false)
		var chunks = [];

		chunks.push(cipher.update(token, 'buffer', 'hex'));
